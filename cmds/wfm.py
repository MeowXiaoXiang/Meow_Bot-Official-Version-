  ...
  async def market(self, ctx, *item):
    ...
    if args.count(',')==0:
        ...
        if order_type == '買':
            ...
            await ctx.send('主人是要買還是賣呢？')
    else:
        await ctx.send("主人說的太多了,Ordis不是很懂呢")
    count = 5
    ...
    try: itemsDetail = json.loads(requests.get("https://api.warframe.market/v1/items/" + item).text.encode(encoding="UTF-8"))["payload"]["item"]["items_in_set"]
      ...
      await ctx.send("Ordis不太清楚主人說的什麼呢")
      return
    max_rank = None
      ...
      if "mod_max_rank" in itemDetail:
        if itemrank is not None:
          ...
          if itemrank > max_rank:
            await ctx.send("主人所輸入的等級超出物品最高等級呢 0.0") 
            return
    url = "https://api.warframe.market/v1/items/" + item + "/orders"
    raw = requests.get(url)
    if raw.status_code != 200:
      await ctx.send("Ordis覺得...主人是不是搞錯了什麼")
      return
    else:
